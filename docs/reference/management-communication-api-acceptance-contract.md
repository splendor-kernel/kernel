...
| `resumeRun` | `/runs/{run_id}/resume` | `POST` | `splendor.runs.control` | Existing run and original bound work order | Resumes a normally paused run only. `waiting_for_approval` must progress through an exact receipt-bearing retry at the original action endpoint (`submitAction` for tick/direct, original-node `submitPhysicalAction` for physical); raw approval evidence and receipt-bearing lifecycle requests are rejected without a tick. |
