...
| `importStateSnapshot` | `/state-snapshots/import` | `POST` | `splendor.state.handoff` | Exact signed work-order envelope admitted for the target run | Validates signature/expiry/revocation, exact tenant/agent/run/work-order authority, receiver instance, schema/mode, previous head, trace linkage, replay, snapshot ID/hash/parents, and imports only through the scheduler/loop/state owner. Failure leaves the live receiver head unchanged. |
