...
| `degraded_mode.allow_low_risk_cached` | Whether disconnected runtimes may allow explicitly listed read-only/low-risk actions from an expired cached bundle. |
...
| Expired cached bundle at runtime | Deny policy invocation and side-effectful actions unless disconnected low-risk cached mode applies to an explicitly listed read-only action. |
