# Legacy Splendor 0.01-0.1 Milestones, FRs, and Isolated Implementation Sprints

> **Status:** Legacy/baseline roadmap detail for the implemented 0.01-0.1 line.
> This file preserves the previous detailed sprint and FR history so maintainers
> do not lose acceptance context. Active 0.2/v2 implementation, QA, integration,
> and gold-example execution rules live in
> [`../sprints_frs_milestones.md`](../sprints_frs_milestones.md) and
> [`../v2/`](../v2/). If any 0.2/v2 wording below conflicts with the active
> entrypoint, the active entrypoint wins.
...
0.2/v2 task IDs touched, if applicable: FND-..., IDR-..., AUTH-..., etc.
Primitives strengthened: ...
```

If a task cannot be tied to at least one FR, do not implement it yet. For
0.2/v2 work, PRs and issues must list both relevant `FR-0.2-*` IDs and the
v2 catalog sprint/task IDs from `docs/rules/v2/0.2-execution-sprints.md` and
...
Splendor0.1-dev:
  Implemented/stable primitive baseline:
  compatibility guarantees + conformance suite + adapter maturity model + SDK/API stability + migration/release policy.

Splendor0.2-dev:
  Active v2 execution:
  agent-kernel v2 task catalog + grouped 0.2 execution sprints under docs/rules/v2.
```
...
| 0.03-S7 | State handoff v0         | Snapshot export/import, state-head validation, read-only reference mode           | Lower-level/explicit local-dev import validates state; resident import remains fail-closed until source-authenticated proof exists. |
...
0.1 is the implemented/stable baseline for compatibility and conformance. Future
0.2/v2 work must preserve 0.1 compatibility or provide accepted RFC-backed
migration paths with fixtures and rollback notes.
...
# 11. Splendor0.2-dev — Active v2 Execution

## 11.1 Goal

Execute the agent-kernel v2 catalog as the active 0.2 work line while preserving
the 0.1 stable baseline, Action Gateway enforcement, explicit state, trace
contract, safe replay, identity separation, secure daemon boundary, no permission
laundering, and high-level physical/device boundary.

## 11.2 Rule Pack

Primary 0.2/v2 rule documents:
...
These files are authoritative only for 0.2/v2 decomposition, sprint sequencing,
task grouping, catalog coverage, dependency gates, component ownership, and
gold/conformance planning. They cannot override accepted RFCs, stable specs,
public schemas, public API contracts, release limitations, AGENTS safety
invariants, gateway/verifier contracts, or RFC requirements. They are not
implementation evidence.

## 11.3 Functional Requirement Bridge

0.2/v2 work must cite both the applicable `FR-0.2-*` bridge IDs below and the
exact v2 sprint/task IDs from `docs/rules/v2/`.
...
the 12 foundations, 38 component owners, 350 component tasks, 19
integration/research/operations programs, and 90 gold cases into reviewable
sprint slices. Do not create 381 individual issues.

## 11.5 Completion Rule

A 0.2/v2 catalog task is complete only when its implementation, tests,
docs/examples, compatibility impact, migration notes where applicable, and
retained validation evidence prove the exact behavior. Moving a catalog row into
`docs/rules/v2/` does not mean the described behavior exists in runtime code.

## 11.6 Non-Goals

```text
no side-effect bypass
no implicit authority from messages, feedback, rewards, evidence, observability, or extensions
...
# 12. Required PR Template for Implementation Agents
...
# 13. Implementation Agent Review Checklist
...
# 15. Suggested Repository Structure Additions
...
# 17. References for Implementation Agents
