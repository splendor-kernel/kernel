...
- [ ] Lower-level/local-dev import cannot proceed unless agent/run/work-order authority is valid.
- [ ] Resident import fails with `state_handoff_proof_unavailable` before mutation even when caller and target work-order authority are valid, until accepted source proof exists.
- [ ] State snapshot hash is verified before import.
- [ ] Read-only state references cannot be mutated by the receiver.
- [ ] Local-dev/lower-level handoff creates trace events on source and receiver sides; resident proof denial creates no receiver run-trace mutation.
...
- No accepted source-signed handoff manifest, source event/evidence proof, or durable cross-instance replay ledger.
