...
evaluation path. Failures report fixture, mutation, and seed.
...
- 128 signed work-order cases exercise missing/blank/forged/unknown-key
  ...
  expiry, revocation, and audience-binding denial through AUTH-002a issuance;
...
| Serialized mutation evidence is bounded and trusted-path checked | Local `AUTH-007b` serializes canonical positive fixtures, mutates `serde_json::Value` deterministically, and requires serde rejection or denial by existing validation/issuance/evaluation. It never constructs unchecked trusted wrappers, mocks signatures/audience/expiry/revocation, or treats deserialization as authority. |
...
| Bounded AUTH-007b deterministic serialized mutation corpus | `cargo test -p splendor-authority serialized_mutation --locked` runs five 128-case families (640 mutations total) over capability grants/requests, signed work-order issuance, obligation receipts, delegation grants/chains, and nested metadata/extensions. Every fixture has a positive control; mutations are checked through serde and the real trusted local boundary with fixture/mutation/seed diagnostics. | `G01/G18/G70/G79/G80/G86/G88` remain `not_exercised`; no cargo-fuzz/libFuzzer, unbounded/random fuzzing, all-plane confused-deputy, cross-version IAM/cache parity, mutation testing, formal proof, external fixture publication, or full AUTH-007/C02/#244 claim. |
