//! Runtime daemon transport and process composition.

use crate::caller_auth::CallerTokenVerifier;
use crate::{router, ConfiguredActionAdapters, DaemonConfig, DaemonState};
...
fn daemon_runtime() -> Result<DaemonRuntime, std::io::Error> {
    daemon_runtime_with_action_adapters(ConfiguredActionAdapters::default())
}

fn daemon_runtime_with_action_adapters(
    action_adapters: ConfiguredActionAdapters,
) -> Result<DaemonRuntime, std::io::Error> {
    match required_env("SPLENDOR_DAEMON_MODE")?.as_str() {
        "local_dev" => {
            ...
            Ok(DaemonRuntime::LocalDev {
                state: DaemonState::with_action_adapters(config, action_adapters),
                bind_addr,
            })
        }
        "resident" => resident_runtime(action_adapters),
...
fn resident_runtime(
    ...
    Ok(DaemonRuntime::Resident {
        state: DaemonState::with_action_adapters(
...
fn read_regular_file(
    ...
    read_bounded_file(file, path, maximum_bytes)
}
...
/// Runs the production daemon with no concrete action provider configured.
pub async fn run_adapterless() -> Result<(), Box<dyn std::error::Error>> {
    run_with_action_adapters(ConfiguredActionAdapters::default()).await
}

/// Runs daemon transport with already-constructed adapters from an outer host.
///
/// This input does not register action authority. Every request still requires
/// signed work-order scope, a trusted action profile, verifier completion, and
/// gateway admission before an injected adapter can execute.
pub async fn run_with_action_adapters(
    action_adapters: ConfiguredActionAdapters,
) -> Result<(), Box<dyn std::error::Error>> {
    match daemon_runtime_with_action_adapters(action_adapters)? {
...
#[cfg(test)]
mod tests {
    use crate::caller_auth::{CallerTokenSigner, CallerTokenTrustSnapshot};
