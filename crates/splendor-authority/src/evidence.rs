...
use crate::{
    canonical_authority_request_digest, evaluate_cached_capability_request,
    evaluate_capability_request, AuthorityConnectivity, AuthorityGrantCache, CachedAuthorityGrant,
    OfflineAuthorityPolicy, RevocationSnapshot, ValidatedCapabilityGrant,
};
...
use splendor_types::{
    ...
    CapabilityRequest, ContentHash, PrincipalId, RevocationStatus,
};
...
pub enum AuthorityEvidenceMissingFact {
    /// Evaluated grant revisions were not supplied with an existing decision.
    EvaluatedGrants,
...
pub enum AuthorityEvidenceWithheldField {
    /// Raw request scope coordinates are represented only by the request digest.
    RequestScopeValues,
...
pub struct AuthorityGrantEvidence {
    ...
    /// Immutable validation digest supplied by the trusted grant wrapper.
    pub validation_digest: String,
    /// Digest over the safe immutable grant revision facts.
...
pub struct AuthorityDecisionEvidence {
    ...
    /// Requested typed operation shape plus digest, without a concrete name.
    pub operation: RedactedAuthorityOperation,
    /// Canonical digest of the exact request, including withheld scope/metadata facts.
    pub canonical_request_digest: String,
    /// Deterministic digest over this recorded decision-evidence fact set.
    ...
    pub evaluated_grants: Vec<AuthorityGrantEvidence>,
...
pub struct RedactedAuthorityOperation {
    ...
    /// Optional target resource schema version.
    pub resource_schema_version: Option<String>,
    /// Digest over the complete typed operation, including a withheld concrete name.
...
pub struct RedactedAuthorityDecisionEvidence {
    ...
    pub explanation: AuthorityDecisionExplanation,
    /// Preserved safe grant refs/digests.
    pub evaluated_grants: Vec<AuthorityGrantEvidence>,
...
impl AuthorityDecisionEvidence {
    ...
    pub fn redacted(&self) -> Result<RedactedAuthorityDecisionEvidence, AuthorityEvidenceError> {
        Ok(RedactedAuthorityDecisionEvidence {
            ...
            evaluated_grants: self.evaluated_grants.clone(),
...
pub struct AuthorityEvidenceComparison {
    ...
    /// Whether the canonical request digest changed.
    pub request_digest_changed: bool,
...
pub fn compare_authority_evidence(
    ...
    AuthorityEvidenceComparison {
        ...
        request_digest_changed: left.canonical_request_digest != right.canonical_request_digest,
...
pub fn authority_reason_category(reason: &str) -> AuthorityExplanationCategory {
    ...
    if matches!(reason.as_str(), "capability_allowed" | "authority_allowed") {
        AuthorityExplanationCategory::Allowed
    } else if reason.contains("data_purpose") || reason.contains("data_use") {
        AuthorityExplanationCategory::DataUse
    } else if reason.contains("revoked") || reason.contains("revocation_ref") {
        ...
        || reason.contains("gate")
    {
        ...
        || reason.contains("missing_from_request")
        || reason.contains("audience")
        || reason.contains("operation")
        || reason.contains("bounded_dimension")
        || reason.contains("tenant_or_fleet")
    {
...
fn build_evidence(
    ...
    mut evaluated_grants: Vec<AuthorityGrantEvidence>,
    ...
    reason_codes.sort();
    reason_codes.dedup();
    let explanation = explanation(&reason_codes);
    ...
    match completeness {
        AuthorityEvidenceCompleteness::DecisionOnly => {
            missing_facts.push(AuthorityEvidenceMissingFact::EvaluatedGrants);
    ...
    let canonical_request_digest =
        canonical_authority_request_digest(&decision.request).map_err(|_| {
            AuthorityEvidenceError::DigestUnavailable {
                reason: "authority_request_digest_unavailable",
            }
        })?;
...
fn explanation(reason_codes: &[String]) -> AuthorityDecisionExplanation {
    let mut branches = Vec::<AuthorityExplanationBranch>::new();
    for reason in reason_codes {
        let category = authority_reason_category(reason);
        if let Some(branch) = branches
            .iter_mut()
            .find(|branch| branch.category == category)
        {
            branch.reason_codes.push(reason.clone());
        } else {
...
fn grant_evidence(
    ...
    let safe_validation_digest = if is_supported_digest(&validation.digest) {
        validation.digest.clone()
    } else {
        // Current local compatibility profiles can carry immutable revision
        // tokens rather than content-hash syntax. Evidence never exports those
        // arbitrary tokens; it exports only a one-way content hash of them.
        ContentHash::blake3(validation.digest.as_bytes()).to_string()
    };
    ...
    let revision = AuthorityGrantRevisionDigestPayload {
        ...
        validation_digest: &safe_validation_digest,
    };
    let revision_digest =
        digest_serializable("authority_grant_revision_digest_unavailable", &revision)?;
...
fn cache_entry_evidence(
    ...
    let freshness = if cached.cached_at() > now {
        AuthorityFreshnessStatus::FutureDated
...
fn redacted_operation(
    ...
    Ok(RedactedAuthorityOperation {
        ...
        verb: operation.verb,
        resource_schema_version: operation.resource_schema_version.clone(),
        operation_digest: digest_serializable("authority_operation_digest_unavailable", operation)?,
    })
}
...
struct AuthorityGrantRevisionDigestPayload<'a> {
    ...
    operations: &'a [AuthorityOperation],
...
struct AuthorityDecisionDigestPayload<'a> {
    ...
    evaluated_grants: &'a [AuthorityGrantEvidence],
...
impl<'a> From<&'a AuthorityDecisionEvidence> for AuthorityDecisionDigestPayload<'a> {
    fn from(value: &'a AuthorityDecisionEvidence) -> Self {
        Self {
            ...
            evaluated_grants: &value.evaluated_grants,
