...
WORK_ORDER_KEY_ID = "work-order-acceptance-key"
SCOPE_VALUES = {
...
def initialize(out: Path) -> None:
    ...
    caller_key = out / "caller-signing-key.pk8"
    caller_public_key = out / "caller-public-key.raw"
    ...
    write_private(out / "work-order-signing.secret", work_order_secret.encode("ascii"))
    write_json(
        out / "work-order-keyring.json",
        {
            "schema_version": "splendor.work_order_keyring.v1",
            "keys": [
                {
                    "key_id": WORK_ORDER_KEY_ID,
    ...
    tls_key = out / "resident-tls-key.pem"
    tls_cert = out / "resident-tls-cert.pem"
    openssl(
        "req",
        "-x509",
        "-newkey",
