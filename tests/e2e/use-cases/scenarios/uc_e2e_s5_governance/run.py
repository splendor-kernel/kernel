...
def manager_credential(scopes: list[str] | None = None) -> dict[str, Any]:
    return {
        ...
        "scopes": scopes or ["policies_publish", "policies_revoke", "approvals_manage", "governance_control", "traces_read", "fleet_read"],
...
def create_run_payload(run_id: str, envelope: dict[str, Any], signed_policy: dict[str, Any] | None, *, policies: list[dict[str, Any]] | None = None, circuit_breakers: list[dict[str, Any]] | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "policy_actions": [{"action": action("artifact.publish_external"), "adapter": "artifact-store", "quota_usage": quota(), "satisfied_preconditions": []}],
...
def main() -> int:
    ...
    expired = call("submitAction", "POST", args.base_url, "/actions", {"run_id": RUN_ID, "tenant_id": TENANT_ID, "agent_id": AGENT_ID, "credential": daemon_cred, "audit_attribution": audit(daemon_cred), "causal_trace_id": "55555555-5555-4555-8555-555555555503", "action": action("artifact.publish_external"), "adapter": "artifact-store", "quota_usage": quota(), "satisfied_preconditions": [], "approval_evidence": expired_evidence})
    revoked_evidence = json.loads(json.dumps(evidence))
    revoked_evidence["approval_id"] = "55555555-5555-4555-8555-555555555504"
    revoked_evidence["revoked"] = True
    revoked = call("submitAction", "POST", args.base_url, "/actions", {"run_id": RUN_ID, "tenant_id": TENANT_ID, "agent_id": AGENT_ID, "credential": daemon_cred, "audit_attribution": audit(daemon_cred), "causal_trace_id": "55555555-5555-4555-8555-555555555505", "action": action("artifact.publish_external"), "adapter": "artifact-store", "quota_usage": quota(), "satisfied_preconditions": [], "approval_evidence": revoked_evidence})
    ...
    if denial_evidence:
        denial_evidence["action_id"] = None
    ...
    breaker = call("createCircuitBreaker", "POST", args.manager_url, "/governance/circuit-breakers", {**sec(manager_cred), "breaker_id": "cb_uc_e2e_s5_artifact_publish", "tenant_id": TENANT_ID, "adapter": "artifact-store", "action": "artifact.publish_external", "reason": "incident_block_external_publish"})
    ...
    call("createRun", "POST", args.base_url, "/runs", create_run_payload(cb_run_id, cb_envelope, signed_policy, policies=[], circuit_breakers=[breaker_obj]))
    ...
    clear = call("clearCircuitBreaker", "POST", args.manager_url, "/governance/circuit-breakers/cb_uc_e2e_s5_artifact_publish/clear", {**sec(manager_cred), "reason": "incident_resolved"})
    clear_wrong_scope = call("clearCircuitBreaker", "POST", args.manager_url, "/governance/circuit-breakers/cb_uc_e2e_s5_artifact_publish/clear", {**sec(manager_credential(["fleet_read"])), "reason": "missing_control_scope"})
    ...
    kill = call("activateKillSwitch", "POST", args.manager_url, "/governance/kill-switches", {**sec(manager_cred), "kill_switch_id": "ks_uc_e2e_s5_run", "run_id": KILL_RUN_ID, "tenant_id": TENANT_ID, "node_id": None, "instance_id": CLOUD_INSTANCE_ID, "reason": "operator_kill_switch", "propagation_ack_required": True, "target_daemon_url": args.cloud_url, "cancel_payload": {"credential": kill_cred, "audit_attribution": audit(kill_cred), "reason": "kill_switch"}})
    ...
    kill_missing_ack = call("activateKillSwitch", "POST", args.manager_url, "/governance/kill-switches", {**sec(manager_cred), "kill_switch_id": "ks_uc_e2e_s5_missing_ack", "run_id": KILL_RUN_ID, "tenant_id": TENANT_ID, "node_id": None, "instance_id": None, "reason": "missing_ack_negative", "propagation_ack_required": True, "target_daemon_url": None, "cancel_payload": None})
    ...
    records = traces["body"].get("records", []) + deny_traces["body"].get("records", []) + cb_traces["body"].get("records", []) + kill_traces["body"].get("records", [])
    ...
    negatives = [
        ...
        {"case": "expired_policy_bundle_fails_closed", "passed": expired_policy_create["status"] == 403 and expired_policy_create["body"].get("code") == "expired_policy_bundle"},
        ...
        {"case": "circuit_breaker_blocks_matching_action", "passed": cb_submit["body"].get("status") == "Denied" and cb_submit["body"].get("verification", {}).get("artifacts", {}).get("circuit_breaker") is not None},
        {"case": "clearing_circuit_breaker_requires_scope", "passed": clear["body"].get("status") == "cleared" and clear_wrong_scope["status"] == 403},
        {"case": "kill_switch_cancels_matching_run", "passed": kill["body"].get("propagation_acknowledged") is True and kill["body"].get("cancel_status") == 200},
    ...
    required_positive = {
        "policy_published": published["status"] == 200 and bool(signed_policy),
        ...
        "approval_granted": approval_request["status"] == 200 and approval_grant["body"].get("status") == "granted",
    ...
    scenario = {
        ...
        "run_ids": [RUN_ID, DENY_RUN_ID, cb_run_id, KILL_RUN_ID],
    ...
    artifacts = {
        ...
        "approval-flow.json": {"request": approval_request["body"], "grant": approval_grant["body"], "denial": denial["body"], "expired": expired["body"], "revoked": revoked["body"]},
        "policy-bundle-report.json": {"published": published["body"], "status": policy_status["body"], "revoked": revoked_policy["body"], "expired_policy_create": expired_policy_create, "revoked_policy_create": revoked_policy_create, "missing_policy_create": missing_policy},
        "circuit-breaker-report.json": {"created": breaker["body"], "blocked_action": cb_submit["body"], "cleared": clear["body"], "clear_wrong_scope": clear_wrong_scope},
